...
// Public: request a login OTP to be sent via Telegram for a phone number
exports.requestLoginOtp = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'No user with that phone' });
    ...
    if (user.telegramChatId) {
      try {
        ...
        const data = botResp && botResp.data ? botResp.data : null;
            if (data && data.ok) {
              const out = { ok: true, via: 'telegram', request_id: String(user._id) };
              if (DEV_RETURN_OTP) out.otp = otp; // dev helper
              return res.json(out);
            }
            return res.json({ ok: false, via: 'telegram', message: 'Bot failed to deliver message' });
      } catch (e) {
        console.warn('Failed to send login OTP via bot:', e && e.message ? e.message : e);
        return res.json({ ok: false, via: 'telegram', message: 'Failed to send via Telegram' });
      }
    }
    const out = { ok: true, via: 'none', message: 'User not linked to Telegram', request_id: String(user._id) };
    if (DEV_RETURN_OTP) out.otp = otp; // dev helper
    return res.json(out);
  } catch (err) {
...
exports.verifyLoginOtp = async (req, res) => {
  try {
    ...
    await user.save();
    const secret = process.env.JWT_SECRET || 'dev_secret';
    const token = jwt.sign({ id: user._id, systemId: user.systemId }, secret, { expiresIn: '30d' });
    return res.json({ id: user._id, fullName: user.fullName, email: user.email, phone: user.phone, token });
  } catch (err) {
